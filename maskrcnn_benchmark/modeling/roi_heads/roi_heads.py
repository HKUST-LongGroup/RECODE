...
class CombinedROIHeadsClip(torch.nn.ModuleDict):
    """
    Combines a set of individual heads (for box prediction or masks) into a single
    head.
    """
    def __init__(self, cfg, heads):
        super(CombinedROIHeadsClip, self).__init__(heads)
        self.cfg = cfg.clone()
    ...
    def forward(self, images, features, proposals, targets=None, logger=None, obj_features=None, spatials=None):
        ...
        if self.feat_type == 'offline':
            ...
            x, detections, loss_box = self.box(images=images, features=features, proposals=proposals, targets=targets)

        # x, detections, loss_box = self.box(images=images, features=features, proposals=proposals, targets=targets)
        if self.type == 'extract_proposal':
            _, extract_proposals, _ = self.box(images=images, features=features, proposals=proposals, targets=targets)
        ...
        if self.cfg.MODEL.RELATION_ON:
            ...
            if self.type == 'extract_clip_feature':
                clip_obj_features = self.relation(images, features, detections, targets, logger)
                return clip_obj_features

            x, detections, loss_relation = self.relation(images, features, detections, targets, logger, obj_features=obj_features, spatials=spatials)
            losses.update(loss_relation)
...
def build_roi_heads(cfg, in_channels):
    ...
    if roi_heads:
        if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'ClipPredictor':
            roi_heads = CombinedROIHeadsClip(cfg, roi_heads)
        else:
