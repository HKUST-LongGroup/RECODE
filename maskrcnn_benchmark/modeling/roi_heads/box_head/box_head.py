...
class ROIBoxHeadClip(torch.nn.Module):
    """
    Generic Box Head class.
    """

    def __init__(self, cfg, in_channels):
        super(ROIBoxHeadClip, self).__init__()
        self.cfg = cfg.clone()
        self.samp_processor = make_roi_box_samp_processor(cfg)
        self.num_obj_classes = cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        from maskrcnn_benchmark.modeling.roi_heads.relation_head.utils_motifs import to_onehot
        self.to_onehot = to_onehot
        # from maskrcnn_benchmark.utils.clip import clip_predictor
        # self.clip_predictor = clip_predictor
    
    def _get_class_logits(self, images, proposals):
        from maskrcnn_benchmark.utils.clip import clip_predictor
        class_logits = clip_predictor(images, proposals)
        return class_logits
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
            ...
            if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL:
                ...
                for img_idx in range(len(proposals)):
                    proposals[img_idx].add_field("predict_logits", self.to_onehot(proposals[img_idx].get_field('labels'), self.num_obj_classes))
                return None, proposals, {}
            else:
                # mode==sgclsq
                # add field:class_logits into gt proposals, note field:labels is still gt
                class_logits = self._get_class_logits(images, proposals)
                proposals = add_predict_logits(proposals, class_logits)
                return None, proposals, {}
        else:
            # mode==sgdet
            ...
            class_logits = self._get_class_logits(images, proposals)
            proposals = add_predict_logits(proposals, class_logits)
            # do not have post process. use bbox from RPN
            return None, proposals, {}
...
def build_roi_box_head(cfg, in_channels):
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'ClipPredictor':
        return ROIBoxHeadClip(cfg, in_channels)
    else:
