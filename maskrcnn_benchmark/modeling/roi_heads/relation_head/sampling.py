...
class RelationSampling(object):
    ...
    def prepare_test_pairs_with_labels(self, device, proposals, targets):
        # prepare object pairs for relation prediction
        rel_pair_idxs = []
        rel_labels = []
        rel_sym_binarys = []
        ...
        # for p in proposals:
            ...
            # mode==sgdet and require_overlap
            if (not self.use_gt_box) and self.test_overlap:
                cand_matrix = cand_matrix.byte() & boxlist_iou(proposal, proposal).gt(0).byte()
            idxs = torch.nonzero(cand_matrix).view(-1,2)
            if len(idxs) > 0:
                rel_pair_idxs.append(idxs)
                tgt_head_idxs = idxs[:, 0].contiguous().view(-1)
                tgt_tail_idxs = idxs[:, 1].contiguous().view(-1)
                tgt_rel_labs = tgt_rel_matrix[tgt_head_idxs, tgt_tail_idxs].contiguous().view(-1)
                rel_labels.append(tgt_rel_labs)
            else:
                # if there is no candidate pairs, give a placeholder of [[0, 0]]
                rel_pair_idxs.append(torch.zeros((1, 2), dtype=torch.int64, device=device))
                rel_labels.append(torch.zeros(1, device=device).long())
        return rel_pair_idxs, rel_labels

    def prepare_test_pairs_with_labels_offline(self, device, obj_features, targets):
        # prepare object pairs for relation prediction
        rel_pair_idxs = []
        rel_labels = []
        rel_sym_binarys = []
        obj_labels = []
        for img_id, (obj_feature, target) in enumerate(zip(obj_features, targets)):
            device = obj_feature.device
            num_prp = obj_feature.shape[0]
            tgt_lab = target.get_field("labels").long()

            assert obj_feature.shape[0] == target.bbox.shape[0]
            tgt_rel_matrix = target.get_field("relation") # [tgt, tgt]
            if obj_feature.shape[0]!=target.bbox.shape[0] or target.bbox.shape[0]!= tgt_lab.shape[0]:
                print(obj_feature.shape[0], target.bbox.shape[0], tgt_lab.shape)
        ...
        # for p in proposals:
            ...
            if len(idxs) > 0:
                ...
                tgt_rel_labs = tgt_rel_matrix[tgt_head_idxs, tgt_tail_idxs].contiguous().view(-1)
                rel_labels.append(tgt_rel_labs)
                obj_labels.append(tgt_lab)
            else:
                # if there is no candidate pairs, give a placeholder of [[0, 0]]
                rel_pair_idxs.append(torch.zeros((1, 2), dtype=torch.int64, device=device))
                rel_labels.append(torch.zeros(1, device=device).long())
                obj_labels.append(tgt_lab)
        return rel_pair_idxs, rel_labels, obj_labels
