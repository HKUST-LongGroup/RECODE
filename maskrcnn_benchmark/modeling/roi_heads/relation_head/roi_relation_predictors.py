...
class ClipPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.cfg = config
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
    def forward(self, images, proposals, rel_pair_idxs=None, rel_labels=None, logger=None, feat_type='offline', obj_labels=None, spatials=None):
        """
        Returns:
            obj_dists (list[Tensor]): logits of object label distribution
            rel_dists (list[Tensor])
        ...
        if feat_type == 'offline':
            num_objs = [len(image) for image in images]
        else:
            num_objs = [len(b) for b in proposals]
        assert len(num_rels) == len(num_objs)

        from maskrcnn_benchmark.utils.clip import clip_predictor
        if feat_type == 'offline':
            if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL != True:
                obj_dist_list, pred_labels = clip_predictor(images, proposals, rel_pair_idxs=rel_pair_idxs, rel_labels=rel_labels, infer_type='obj_cls', feat_type=feat_type, obj_labels=obj_labels, spatials=spatials)
                rel_dist_list = clip_predictor(images, proposals, rel_pair_idxs=rel_pair_idxs, rel_labels=rel_labels, infer_type='rel_cls', feat_type=feat_type, obj_labels=pred_labels, spatials=spatials)
            else:
                obj_dist_list = [to_onehot(obj_label, self.num_obj_cls) for obj_label in obj_labels]
                rel_dist_list = clip_predictor(images, proposals, rel_pair_idxs=rel_pair_idxs, rel_labels=rel_labels, infer_type='rel_cls', feat_type=feat_type, obj_labels=obj_labels, spatials=spatials)
            
        else:        
            obj_dist_list = [proposal.get_field('predict_logits') for proposal in proposals]
            rel_dist_list = clip_predictor(images, proposals, rel_pair_idxs=rel_pair_idxs, rel_labels=rel_labels, infer_type='rel_cls', feat_type=feat_type, obj_labels=obj_labels, spatials=spatials)
        
        return obj_dist_list, rel_dist_list, {}

    @torch.no_grad()
    def forward_extract_feature(self, images, proposals):
        from maskrcnn_benchmark.utils.clip import clip_predictor
        clip_obj_features = clip_predictor.forward_extract_feature(images, proposals)
