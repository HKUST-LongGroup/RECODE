...
class ROIRelationHeadClip(torch.nn.Module):
    """
    Generic Relation Head class.
    """

    def __init__(self, cfg, in_channels):
        super(ROIRelationHeadClip, self).__init__()
        self.cfg = cfg.clone()
        self.predictor = make_roi_relation_predictor(cfg, -1)
        self.post_processor = make_roi_relation_post_processor(cfg)
        self.samp_processor = make_roi_relation_samp_processor(cfg)
    ...
    def forward(self, images, features, proposals, targets=None, logger=None, obj_features=None, spatials=None):
        """
        Arguments:
            features (list[Tensor]): feature-maps from possibly several levels
            proposals (list[BoxList]): proposal boxes. Note: it has been post-processed (regression, nms) in sgdet mode
        ...
        if self.type == 'extract_clip_feature':
            # rel_pair_idxs, rel_labels = self.samp_processor.prepare_test_pairs_with_labels(features[0].device, proposals, targets=targets)
            clip_obj_features = self.predictor.forward_extract_feature(images, proposals)
            return clip_obj_features
        else:
            if self.feat_type == 'offline':
                rel_pair_idxs, rel_labels, obj_labels = self.samp_processor.prepare_test_pairs_with_labels_offline(obj_features[0].device, obj_features, targets=targets)
                if self.cfg.TYPE == 'extract_spatial':
                    # for rel_pair_idx in rel_pair_idxs:
                    pass       
                else:
                    refine_logits, relation_logits, add_losses = self.predictor(obj_features, proposals, rel_labels=rel_labels, rel_pair_idxs=rel_pair_idxs, logger=logger, feat_type=self.feat_type, obj_labels=obj_labels, spatials=spatials)
            else:
                rel_pair_idxs, rel_labels = self.samp_processor.prepare_test_pairs_with_labels(features[0].device, proposals, targets=targets)
                refine_logits, relation_logits, add_losses = self.predictor(images, proposals, rel_labels=rel_labels, rel_pair_idxs=rel_pair_idxs, logger=logger, feat_type=self.feat_type, spatials=spatials)
...
def build_roi_relation_head(cfg, in_channels):
    ...
    """
    if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'ClipPredictor':
        return ROIRelationHeadClip(cfg, in_channels)
    else:
