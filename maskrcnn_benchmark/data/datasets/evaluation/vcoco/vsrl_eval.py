...
#   # vcocoeval = VCOCOeval('//home/chenguikun/datasets/v-coco/annotations/test_vcoco.json',
#   #                       '/home/chenguikun/datasets/v-coco/data/instances_vcoco_all_2014.json',
#   #                       '/PATH/TO/data/v-coco/data/splits/vcoco_test.ids')
#     import sys
#     vsrl_annot_file = '/home/chenguikun/projects/v-coco/data/vcoco/vcoco_test.json'
#     coco_file = '/home/chenguikun/datasets/v-coco/data/instances_vcoco_all_2014.json'
#     split_file = '/home/chenguikun/projects/v-coco/data/splits/vcoco_test.ids'

#     vcocoeval = VCOCOeval(vsrl_annot_file, coco_file, split_file)
#     det_file = '/home/chenguikun/projects/Zero_Shot_SGG/vcoco.pickle'
...
class VCOCOeval(object):
  ...
  def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.5, eval_type='scenario_1'):
    ...
    #   if len(self.roles[aid])<2: continue
    #   for rid in range(len(self.roles[aid])-1):
    #     if self.roles[aid][rid+1] != 'instr':
    #       print('{: >23}: AP = {:0.2f} (#pos = {:d})'.format(self.actions[aid]+'-'+self.roles[aid][rid+1], role_ap[aid, rid]*100.0, int(npos[aid])))
    #       filter_ap.append(role_ap[aid, rid])
    # print('Average Role [%s] AP = %.2f'%(eval_type, np.nanmean(np.array(filter_ap)) * 100.00))  
    # print(filter_ap)
    # print('---------------------------------------------') 
    # print('Average Role [%s] AP = %.2f, omitting the action "point"'%(eval_type, (np.nanmean(np.array(filter_ap)) * len(filter_ap) - role_ap[-3][0]) / (len(filter_ap)-1) * 100.00))  
    # print('---------------------------------------------') 
...
if __name__ == '__main__':
  ...
  #                       '/PATH/TO/data/v-coco/data/splits/vcoco_test.ids')
    ...
    vcocoeval = VCOCOeval(vsrl_annot_file, coco_file, split_file)
    det_file = '/home/chenguikun/projects/Zero_Shot_SGG/cop_vcoco_filter_gptw_False_spatialscale0.6_False_0.1.pickle'
    # det_file = '/home/chenguikun/projects/Zero_Shot_SGG/vcoco_filter_spatialscale0.6_True_clip_0.1.pickle'
    # det_file = '/home/chenguikun/projects/Zero_Shot_SGG/vcoco_filter_gptw_False_spatialscale0.6_True_0.1.pickle'
