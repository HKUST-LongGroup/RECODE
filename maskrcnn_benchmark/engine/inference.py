...
def compute_on_dataset(model, data_loader, device, synchronize_gather=True, timer=None):
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        with torch.no_grad():
            ...
            if cfg.TEST.BBOX_AUG.ENABLED:
                ...
                use_blip = True
                if cfg.FEAT_TYPE == 'offline':
                    if cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL == True:
                        mode = 'predcls'
                    elif cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX== True:
                        mode = 'sgcls'
                    else:
                        mode = 'sgdet'
                    ...
                    for image_id in image_ids:
                        obj_features.append(clip_obj_feature[image_id].to(device))
                        proposals.append(box_proposals[image_id].to(device))
                        spatials.append(spatial_infos[image_id])
                    output = model(images.to(device), targets, obj_features=obj_features, proposals=proposals, spatials=spatials)
                else:
    ...
    if cfg.TYPE == 'extract_prompts':
        torch.save(saved_pred_sub_dict, os.path.join(cfg.GPT_DIR, "saved_pred_sub.pth"))
        torch.save(saved_pred_obj_dict, os.path.join(cfg.GPT_DIR, "saved_pred_obj.pth"))
    torch.cuda.empty_cache()
